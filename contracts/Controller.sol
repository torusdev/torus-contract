...
contract Controller is IController, Ownable {
    ...
    address public immutable torusToken;
    ...
    constructor(address torusToken_, address curveRegistryCacheAddress_) {
        torusToken = torusToken_;
        curveRegistryCache = ICurveRegistryCache(curveRegistryCacheAddress_);

        EmergencyMinter emergencyMinter_ = new EmergencyMinter(ITORUSToken(torusToken_), this);
    ...
    function shutdownPool(address poolAddress) external override onlyOwner {
        ...
        ITorusPool(poolAddress).shutdownPool();
    ...
    function updateWeights(WeightUpdate memory update) public override onlyOwner {
        require(
            lastWeightUpdate[update.torusPoolAddress] + weightUpdateMinDelay < block.timestamp,
            "weight update delay not elapsed"
        );
        ITorusPool(update.torusPoolAddress).updateWeights(update.weights);
        lastWeightUpdate[update.torusPoolAddress] = block.timestamp;
    }
