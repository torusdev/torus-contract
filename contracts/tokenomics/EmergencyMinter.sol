...
/// @notice this contract will be added as a minter to the TORUS token
...
contract EmergencyMinter is Ownable {
    ...
    constructor(ITORUSToken _torus, IController _controller) {
        torus = _torus;
        controller = _controller;
        deployedAt = block.timestamp;
    }

    /// @notice this switches the rebalancing reward handler in charge of minting TORUS as reward
    ...
    function _switchMinter(IMinter currentMinter, IMinter replacementMinter) internal {
        ...
        torus.addMinter(address(replacementMinter));
    }
    ...
    function _isMinter(address minter) internal view returns (bool) {
        address[] memory minters = torus.listMinters();
